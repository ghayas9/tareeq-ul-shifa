...
import { BrandState, Brand, BrandWithProductCount, PaginatedResponse } from '@/types/brandTypes';
import {
    createBrandThunk,
    updateBrandThunk,
    getAllBrandsThunk,
...
const brandSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(createBrandThunk.fulfilled, (state, action: PayloadAction<Brand>) => {
        ...
        builder.addCase(updateBrandThunk.fulfilled, (state, action: PayloadAction<Brand>) => {
            state.isLoading = false;
        ...
        builder.addCase(getAllBrandsThunk.fulfilled, (state, action: PayloadAction<PaginatedResponse<Brand>>) => {
            state.isLoading = false;
        ...
        builder.addCase(getBrandByIdThunk.fulfilled, (state, action: PayloadAction<Brand>) => {
            state.isLoading = false;
            state.currentBrand = action.payload;
        });
        ...
        builder.addCase(deleteBrandThunk.fulfilled, (state, action: PayloadAction<{ id: string } | { message: string }>) => {
            state.isLoading = false;
        ...
        builder.addCase(bulkDeleteBrandsThunk.fulfilled, (state, action: PayloadAction<{ ids?: string[] } | { message: string }>) => {
            state.isLoading = false;
            ...
            if (deletedIds.length > 0) {
                state.brands = state.brands.filter(brand => !deletedIds.includes(brand.id));
                if (state.currentBrand && deletedIds.includes(state.currentBrand.id)) {
                    state.currentBrand = null;
                }
            }
        });
        ...
        builder.addCase(updateBrandDiscountThunk.fulfilled, (state, action: PayloadAction<Brand>) => {
            state.isLoading = false;
            const index = state.brands.findIndex(brand => brand.id === action.payload.id);
            if (index !== -1) {
                state.brands[index] = action.payload;
        ...
        builder.addCase(getBrandsWithProductCountThunk.fulfilled, (state, action: PayloadAction<PaginatedResponse<BrandWithProductCount>>) => {
            state.isLoading = false;
            state.brandsWithProductCount = action.payload.data;
            state.meta = action.payload.meta;
        });
        builder.addCase(getBrandsWithProductCountThunk.rejected, (state, action) => {
            state.isLoading = false;
            state.error = action.payload as string;
        });
    }
...
export const { clearBrandError, clearCurrentBrand, setCurrentBrand } = brandSlice.actions;
